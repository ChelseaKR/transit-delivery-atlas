...
/**
 * The closing clause about the next check, expired against the build date.
 *
 * `buildDate` is required, and deliberately so. An optional reference date
 * would let a caller silently restore the behaviour this exists to remove: the
 * sentence used to be built unconditionally, so every directive published "The
 * next planned check of the listed sources is <date>" as a forward-looking
 * commitment for as long as the data sat unreviewed. `lib/watchlist-review.mjs`
 * already states the rule --- "a planned review date that cannot expire is
 * decoration" --- and the release gate already treats the evidence collection
 * as a reviewed item under it. Only the render half was missing, which is why
 * `docs/EVIDENCE-MODEL.md` promised behaviour that half existed.
 *
 * The wording matches `components/WatchlistCard.tsx` on purpose: one rule, one
 * sentence, two layers.
 *
 * @param {DirectiveCoverage} coverage
 * @param {string} buildDate ISO calendar date the published bytes were built
 * @returns {string}
 */
export function nextCheckSentence(coverage, buildDate) {
  if (typeof buildDate !== "string" || !isIsoDate(buildDate)) {
    throw new Error(
      "coverageStatement requires the build date as an ISO calendar date: a " +
        "planned review date that cannot expire is decoration (received " +
        `${JSON.stringify(buildDate)}).`,
    );
  }
  // `?? null`, not `?? buildDate`. Substituting the build date for a source that has
  // never been successfully checked told `reviewCurrency` it was checked today, and it
  // duly reported `daysSinceReview: 0`. Nothing renders that field today, so the sentence
  // below was right by luck; the next reader of it would have been wrong, and the whole
  // point of this layer is that an unchecked source says so.
  const review = reviewCurrency(
    { lastReviewedOn: coverage.lastCheckedOn ?? null, nextReviewOn: coverage.nextReviewOn },
    buildDate,
  );
  if (!review.overdue) {
    return `The next planned check of the listed sources is ${coverage.nextReviewOn}.`;
  }
  const days = `${review.daysOverdue} day${review.daysOverdue === 1 ? "" : "s"}`;
  return (
    `The planned check of the listed sources was due ${coverage.nextReviewOn} and ` +
    `is ${days} overdue at this build (${buildDate}), so this coverage has not ` +
 ...
 * @returns {string}
 */
export function coverageStatement(coverage, buildDate) {
  const failed =
    coverage.failedSources.length > 0
      ...
      : "";
  const next = ` ${nextCheckSentence(coverage, buildDate)}`;
