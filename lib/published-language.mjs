 ...
 * normalized, and only into a placeholder, so no other word can drift past the
 * comparison.
 */
const DATE_PATTERNS = [
...
/**
 * The comparison key for the registry: normalized text with build-relative
...
export function registryKey(sentence) {
  let key = normalizeWhitespace(sentence);
...
export function publishedSentences(html) {
  ...
  for (const run of runs) {
    const text = normalizeWhitespace(run);
    if (text) sentences.push(...splitIntoSentences(text));
  }
...
export const PERMITTED_VERDICT_SENTENCES = Object.freeze([
  ...
  },
  {
    ...
    reason:
      "A labeled inferred delivery dependency for directive 1(c). 'federal compliance procedures' is the name of a process the order's subject matter involves, not a claim that anyone complied.",
  },
  {
    sentence:
    ...
    reason:
      "The same labeled dependency where the directive page renders its inference label immediately after it.",
  },
  {
    sentence: "Q2 How do compliance duties and reversibility work after an election?",
    reason:
      "A labeled open question about how the order's compliance duties are defined. It asks what the duties are; it does not answer whether they were met.",
  },
  {
    sentence:
