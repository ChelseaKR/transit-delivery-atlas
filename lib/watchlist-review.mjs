 ...
 * @property {string | null} lastReviewedOn null when nothing has been checked yet
 ...
 * @property {number | null} daysSinceReview whole days since the last manual check;
 *   null when there has been no check, which is not the same as zero days since one
 * @property {boolean} beyondGrace overdue by more than {@link REVIEW_GRACE_DAYS}
 * @property {number} graceDays
 */

/**
 * `lastReviewedOn` may be null, and only null --- never a substituted date. A caller
 * that had no check date used to pass the build date in its place, which reported
 * `daysSinceReview: 0`: "checked today" for something never checked. Absence travels
 * through as absence instead, so a reader of that field gets null rather than a number
 * no observation supports.
 *
 * @param {{ id?: string, lastReviewedOn: string | null, nextReviewOn: string }} item
...
export function reviewCurrency(item, buildDate) {
  ...
  const lastReviewedOn = item.lastReviewedOn ?? null;
  const nextReviewOn = item.nextReviewOn;
  if (lastReviewedOn !== null) toUtcTime(lastReviewedOn, `${label} lastReviewedOn`);
  ...
  return {
    ...
    daysSinceReview:
      lastReviewedOn === null ? null : daysBetweenIsoDates(lastReviewedOn, buildDate),
...
export function overdueReviewReport(overdue, buildDate) {
  return [
    `${overdue.length} reviewed item(s) (context-watchlist items or the evidence collection) are past their planned review date as of ${buildDate}:`,
    ...overdue.map(
    ...
    `Re-review the source and update lastReviewedOn and nextReviewOn (and evidenceBoundary.checkedOn for a watchlist item, or the review sources and sweep log for the evidence collection).`,
