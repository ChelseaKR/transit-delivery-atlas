 ...
 * without a layer is a `TypeError` here rather than an uncategorized entry in a
 * published feed.
 */
...
/**
 * Build one entry, refusing a kind that has no layer.
 *
 * Exported so the refusal is testable. Nothing in the committed data can reach
 * it — every call site here passes a literal kind — but it is the contract for
 * anyone adding a kind later, and a guard nothing can exercise is a guard
 * nobody knows works.
 *
 * @param {Omit<ChangeEntry, "id" | "layer">} fields
 * @returns {ChangeEntry}
 */
export function changeEntry({ kind, date, recordId, path, directiveIds, title, detail, observedBy }) {
  const layer = CHANGE_KINDS[kind];
  if (layer === undefined) {
    throw new TypeError(`${kind} is not a change kind this build knows how to categorize.`);
  }
