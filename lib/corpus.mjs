...
/**
 * The page markers this project writes into the retained text.
 *
 * `\d+`, not `\d`. The single-digit form matched `=== PAGE 5 ===` and not
 * `=== PAGE 10 ===`, which is inert on a five-page order and wrong on the first
 * instrument longer than nine pages: `pageOfQuote` would carry the last matched
 * page forward and publish a locator pointing at the wrong page, and
 * `quoteIsVerbatim` would leave the marker text sitting in the haystack a
 * quotation is matched against. Both are the atlas's own argument -- a quotation
 * resolves to a page -- failing quietly.
 *
 * Declared once so the two uses cannot drift apart. Each carries its own flags,
 * because a `g`-flagged regex is stateful and sharing one object between a
 * `replace` and a `split` would be a bug of a different kind.
 */
export const PAGE_MARKER_GLOBAL = /=== PAGE \d+ ===/g;
export const PAGE_MARKER_CAPTURING = /=== PAGE (\d+) ===/;
...
/**
 * Normalize text so that a quotation can be compared to an OCR'd page without
 ...
 * than `minimumFragmentLength` normalized characters is rejected outright: a
...
export function quoteIsVerbatim(quote, text, options = {}) {
  ...
  const haystack = normalizeForQuote(text.replace(PAGE_MARKER_GLOBAL, " "));
  let cursor = 0;
...
/**
 * Page number (1-based) at which a normalized fragment first appears, from the
...
export function pageOfQuote(quote, text) {
  ...
  // Build one normalized haystack while remembering where each page starts,
  // so a quotation that begins on one page and runs onto the next is placed
  // on the page it begins on.
  const parts = text.split(PAGE_MARKER_CAPTURING).slice(1);
  const pageStarts = [];
  let haystack = "";
